...
void initsnake()
{
    ...
    {
        g_psnake = (snake*)malloc(sizeof(snake)); //初始化蛇头
        g_psnake->next = tail;        //蛇头的下一位为蛇尾
        g_psnake->x = 24 + 2 * i;         //设置蛇头位置
        g_psnake->y = 5;
        tail = g_psnake;              //蛇头变成蛇尾，然后重复循环
    }
...
int biteself()
{
    ...
    self = g_psnake->next;        //self是蛇头之外的蛇身上的节点
    while (self != NULL)
    {
        if (self->x == g_psnake->x && self->y == g_psnake->y)    //如果self和蛇身上的节点重合
        {
            return 1;       //返回1
        }
        self = self->next;
...
void cantcrosswall()
{
    if (g_psnake->x == 0 || g_psnake->x == 56 || g_psnake->y == 0 || g_psnake->y == 26) //如果蛇头碰到了墙壁
    {
...
void snakemove()	//蛇前进,上U,下D,左L,右R
{
    snake* nextg_psnake;
    cantcrosswall();
    nextg_psnake = (snake*)malloc(sizeof(snake));		//为下一步开辟空间
    if (status == U)
    {
        nextg_psnake->x = g_psnake->x;        //向上前进时，x坐标不动，y坐标-1
        nextg_psnake->y = g_psnake->y - 1;
        nextg_psnake->next = g_psnake;
        g_psnake = nextg_psnake;
        q = g_psnake;                 //指针q指向蛇头
        if (nextg_psnake->x == food->x && nextg_psnake->y == food->y)	//如果下一个有食物 下一个位置的坐标和食物的坐标相同
        {
    ...
    if (status == D)
    {
        nextg_psnake->x = g_psnake->x;        //向下前进时，x坐标不动，y坐标+1
        nextg_psnake->y = g_psnake->y + 1;
        nextg_psnake->next = g_psnake;
        g_psnake = nextg_psnake;
        q = g_psnake;
        if (nextg_psnake->x == food->x && nextg_psnake->y == food->y)  //有食物
        {
    ...
    if (status == L)
    {
        nextg_psnake->x = g_psnake->x - 2;        //向左前进时，x坐标向左移动-2，y坐标不动
        nextg_psnake->y = g_psnake->y;
        nextg_psnake->next = g_psnake;
        g_psnake = nextg_psnake;
        q = g_psnake;
        if (nextg_psnake->x == food->x && nextg_psnake->y == food->y)//有食物
    ...
    if (status == R)
    {
        nextg_psnake->x = g_psnake->x + 2;        //向右前进时，x坐标向右移动+2，y坐标不动
        nextg_psnake->y = g_psnake->y;
        nextg_psnake->next = g_psnake;
        g_psnake = nextg_psnake;
        q = g_psnake;
